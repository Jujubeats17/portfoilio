...
if sub_or_bus != "subway" and sub_or_bus != "bus":
    print("That's not a valid answer! Try again.")
    sub_or_bus = input("Do you want to take the bus or the subway?")
    print(sub_or_bus)

if sub_or_bus == "subway" or sub_or_bus == " subway":
    time += 5
    loc_or_ex = input("Do you want to take express (10 min wait time) or local (2 min wait time)?")

    if loc_or_ex != "express" and loc_or_ex != "local":
        print("Not a valid answer! Try again.")
        loc_or_ex = input("Do you want to take express (10 min wait time) or local (2 min wait time)?")


        if loc_or_ex == "express" or loc_or_ex == " express": #This is the code for the express train
            time += 10
            stay_or_go = input("There's a sick passenger on the train. You can choose to get off or stay, which do you do?")
            if stay_or_go != "get off" and stay_or_go != "stay":
                stay_or_go = input("There's a sick passenger on the train. You can choose to get off or stay, which do you do?")
            if stay_or_go == "get off":
                time += 45
                print("It takes a really long time to find another train since there were also delays on that line. You finally get off and take a 5 minute walk to GWC.")
                time += 5
        ...
        if loc_or_ex == "local"or loc_or_ex == " local": #This is the code for the local train
            time += 12
            ...
            if left_or_right != "left" and left_or_right != "right":
                left_or_right = input("You exit and the sidewalks you usually use are blocked. Do you go left or right?")
            if left_or_right != "right" or left_or_right !=
                print("You walked around and couldn't find a route and ended up where you started.")
                time += 10
                left_or_right = input("You exit and the sidewalks you usually use are blocked. Do you go left or right?")
            if left_or_right == "right" or left_or_right == " right" and time < 61:
                time += 5
...
if sub_or_bus == "bus":
    ...
    if wait_or_geton != "wait" or "get on":
        print("That's not a valid answer! Try again.")
        wait_or_geton = input("There's a fast bus 10 minutes away and a slower bus at the station. Do you wait or get on?")
    ...
    if wait_or_geton == "wait":
        ...
        if stay_or_getoff != "stay" or "get off":
            print("That's not a valid answer. Try again.")
            stay_or_getoff = input("The bus driver knocks off someone's rearview mirror and now they're arguing with the car's owner. Do you stay or get off?")
        ...
        if stay_or_getoff == "stay":
            print("There's a heated argument between the bus driver and the car driver that is very similar to reality show arguements. It's VERY entertaining, but unfortunately takes just as long as a TV show. You wait 30 minutes.")
            time += 30
    ...
    if wait_or_geton == "get on":
        ...
        while fight_or_flight != "ignore him" or "stand up":
            print("That's not a valid answer! Try again.")
            fight_or_flight = input("After 10 minutes, a guy comes on the bus and starts harassing a woman and threatening to beat her up. Do you ignore him or do you stand up for the woman?")
